	...
	for bo.Ongoing() {
		...
		if err == nil {
			...
			if jobCheckErr != nil {
				...
				finished, err := t.jobContainerFinishedLogging(ctx)
				if finished {
					level.Info(t.log).Log("msg", "job container finished logging, stopping tailer")
					return
				} else if err != nil {
					level.Warn(t.log).Log("msg", "could not determine if job container finished logging; will retry tailing", "err", err)
				}
			} else {
...
// be used for regular pods. Job pods should use jobContainerFinishedLogging()
...
// jobContainerFinishedLogging determines if a job container has finished
// producing logs. This function implements a more robust strategy for job pods:
//
...
func (t *tailer) jobContainerFinishedLogging(ctx context.Context) (finished bool, err error) {
	var (
		key           = t.target.NamespacedName()
		containerName = t.target.ContainerName()
	)
